...
export const viewVariables = {
  ...
  controlPeriodocity: {
    ...
    values: [3650, 365, 182, 91, 30, 7, 1, 0.1, 'NA'],
...
export function createColorScale (variable) {
  const t = viewVariables[variable];

  if (variable === 'controlPeriodocity') {
    const scale = d3.scaleSequentialLog(d3.interpolate('orange', 'purple'))
      .domain(d3.extent(t.values));

    const interp = [3650, 365, 182, 91, 30, 7, 1, 0.1].map(d => scale(d));
